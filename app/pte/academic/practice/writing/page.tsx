...
import { getWritingQuestionTypeStats } from '@/lib/pte/queries-enhanced'
...
export default async function WritingPracticePage() {
  const [summarizeStats, essayStats] = await Promise.all([
    getWritingQuestionTypeStats('summarize_written_text'),
    getWritingQuestionTypeStats('write_essay'),
  ])
  ...
  const categories = [
    {
      ...
      icon: FileText,
      count: summarizeStats.totalQuestions || 0,
    ...
    },
    {
      ...
      icon: PenTool,
      count: essayStats.totalQuestions || 0,
