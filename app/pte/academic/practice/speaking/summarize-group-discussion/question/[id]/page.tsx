    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
          Summarize Group Discussion Practice — Question {id.slice(0, 8)}
        </h1>
        {question.difficulty && (
      ...
      <div className="flex items-center justify-between gap-3 border-t pt-6">
        <div className="flex items-center gap-2">
          <Button asChild variant="outline">
            <Link href="/pte/academic/practice/speaking/summarize-group-discussion">
              ← Back to List
            </Link>
          </Button>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button asChild variant="outline">
            <Link
              href={`/pte/academic/practice/speaking/summarize-group-discussion/question/${id}`}
            >
              Redo
            </Link>
          </Button>
...
export async function generateMetadata(props: Params) {
  ...
  const id = params.id
  return {
    title: `Summarize Group Discussion Practice — Question ${id.slice(0, 8)}`,
    description: 'Practice PTE Academic Summarize Group Discussion with AI scoring',
  }
}

export default async function SummarizeGroupDiscussionQuestionPage(props: Params) {
  const params = await props.params
  const id = params.id
    ...
    <div className="bg-background min-h-screen">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <nav className="text-muted-foreground mt-4 flex items-center gap-2 text-sm">
          ...
          <span>/</span>
          <Link
            href="/pte/academic/practice/speaking/summarize-group-discussion"
            className="hover:text-primary"
          >
