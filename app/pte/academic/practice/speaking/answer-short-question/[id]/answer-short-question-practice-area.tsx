...
interface AnswerShortQuestionPracticeAreaProps {
  questionId: string
  audioUrl?: string // Audio of the question
}

export default function AnswerShortQuestionPracticeArea({
  questionId,
  audioUrl,
}: AnswerShortQuestionPracticeAreaProps) {
  ...
  const playQuestionAudio = useCallback(() => {
    if (audioUrl) {
      ...
      questionAudioRef.current.onplay = () => setIsPlayingQuestion(true)
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Question & Your Answer</CardTitle>
          <CardDescription>
            Listen to the question and provide a short answer.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center space-x-2">
            <Button
              ...
              disabled={isPlayingQuestion || isRecording || isPlayingRecorded || !audioUrl}
              variant="outline"
            >
              <Volume2 className="h-5 w-5 mr-2" />
              {isPlayingQuestion ? 'Playing Question...' : 'Play Question'}
            </Button>
          </div>
          <Separator />
          <div className="flex items-center space-x-2">
            <Button
              ...
              disabled={isPlayingQuestion || isPlayingRecorded}
            >
              {isRecording ? (
                <>
            ...
            {recordedAudioURL && (
              <Button
                ...
                disabled={isPlayingQuestion || isRecording || isPlayingRecorded}
                variant="outline"
              >
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Your answer will be analyzed for content accuracy, pronunciation, and fluency.
