...
interface RespondToASituationPracticeAreaProps {
  questionId: string
  audioUrl?: string // Audio of the situation description
}

export default function RespondToASituationPracticeArea({
  questionId,
  audioUrl,
}: RespondToASituationPracticeAreaProps) {
  const [isRecording, setIsRecording] = useState(false)
  const [recordedAudio, setRecordedAudio] = useState<Blob | null>(null)
  const [recordedAudioURL, setRecordedAudioURL] = useState<string | null>(null)
  const [isPlayingRecorded, setIsPlayingRecorded] = useState(false)
  const [isPlayingSituation, setIsPlayingSituation] = useState(false)
  const mediaRecorderRef = useRef<MediaRecorder | null>(null)
  const audioChunksRef = useRef<Blob[]>([])
  const recordedAudioRef = useRef<HTMLAudioElement | null>(null)
  const situationAudioRef = useRef<HTMLAudioElement | null>(null)
  ...
  const playSituationAudio = useCallback(() => {
    if (audioUrl) {
      if (situationAudioRef.current) {
        situationAudioRef.current.pause()
        situationAudioRef.current.currentTime = 0
      }
      situationAudioRef.current = new Audio(audioUrl)
      situationAudioRef.current.onplay = () => setIsPlayingSituation(true)
      situationAudioRef.current.onended = () => setIsPlayingSituation(false)
      situationAudioRef.current.play().catch((e) => {
        console.error('Error playing situation audio:', e)
        setIsPlayingSituation(false)
      })
    }
  }, [audioUrl])
  ...
  useEffect(() => {
    return () => {
      ...
      if (situationAudioRef.current) {
        situationAudioRef.current.pause()
        situationAudioRef.current = null
      }
    }
  }, [recordedAudioURL])
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Situation & Your Response</CardTitle>
          <CardDescription>
            Listen to the situation and respond as you would naturally.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center space-x-2">
            <Button
              onClick={playSituationAudio}
              disabled={isPlayingSituation || isRecording || isPlayingRecorded || !audioUrl}
              variant="outline"
            >
              <Volume2 className="h-5 w-5 mr-2" />
              {isPlayingSituation ? 'Playing Situation...' : 'Play Situation'}
            </Button>
          </div>
          <Separator />
          <div className="flex items-center space-x-2">
            <Button
              ...
              disabled={isPlayingSituation || isPlayingRecorded}
            >
              {isRecording ? (
                <>
            ...
            {recordedAudioURL && (
              <Button
                ...
                disabled={isPlayingSituation || isRecording || isPlayingRecorded}
                variant="outline"
              >
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Your response will be analyzed for content, appropriateness, pronunciation, and fluency.
